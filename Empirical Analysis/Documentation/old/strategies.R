#---------------------------------------------------------CS
cmom <- function(data, scaled=F, netlong=0){
  ...
  for(r in rankper){
    for(h in holdper){
      for(mon.index in 2:(nrow(x)-r-h+1)){ #mon.index is the start period of the rankper, only runs to that
        ...
        th <- sum(prod.ret.rp[is.na(prod.ret.rp)==F]*weights[is.na(prod.ret.rp)==F])/sum(weights[is.na(prod.ret.rp)==F])  #threshhold for CSMOM (here: average in rankper)
       ...
       #cmom[mon.index,c] <- 0.5*sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F])/length(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F])*
          #(-sum(prod.ret.hp[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp<th])*weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp<0]/sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp<0])
           #+ sum(prod.ret.hp[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=th])*weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=0]/sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=0]))
       #---------------------my formula:
        cmom[mon.index,c] <- -sum(prod.ret.hp[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp<th]*weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp<th])/sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp<th]) + sum(prod.ret.hp[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=th]*weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=th])/sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=th])
        if(netlong==1){
          netlongpos[mon.index,c] <- 2*(sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=0])-sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp<0]))/sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F])
          #---------------------my formula:
          ...
          #short[mon.index,c] <- 1 + netlongpos[mon.index,c]*(netlongpos[mon.index,c]<0)
            ...
            if(netlong==2){
          cmom[mon.index,c] <- 2/sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F])*(-sum(prod.ret.hp[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp<th]*weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp<th])+ sum(prod.ret.hp[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=th])*weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=th])

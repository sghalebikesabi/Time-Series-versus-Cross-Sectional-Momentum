...
cmom <- function(data=ps, scaled=F, netlong=0, th=5, calc.turnover=F){
  ...
  for(r in rankper){
    for(h in holdper){
      ...
      for(mon.index in 2:(nrow(x)-r-h+1)){
        ...
        if(netlong==1 | netlong==4){
          ...
          netlongpos[mon.index,c] <- (sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=0 & is.na(weights)==F])
                                      -sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp<0 & is.na(weights)==F]))/sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & is.na(weights)==F])
          tvm[mon.index,c] <- th
        ...
        if(netlong==2){
          ...
          cmom[mon.index,c] <- 0
          long[mon.index,c] <-  sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=th & is.na(weights)==F])
          if(length(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=th & is.na(weights)==F])>0) {
      ...
      if(netlong==0){
        ...
        if(scaled==T) {
          companies.long.new <- companies.long.new*mean(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & is.na(weights)==F])
          companies.short.new <- companies.short.new*mean(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & is.na(weights)==F])
        }
      }      
      if(netlong==2){
      ...
      if(netlong==1){
        netlongpos[mon.index,c] <- (sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=0 & is.na(weights)==F])-sum(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp<0 & is.na(weights)==F]))/2*mean(weights[is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & is.na(weights)==F])
        ...
        turnover[mon.index,c] <- turnover[mon.index,c] + netlongpos[mon.index,c] - old
  ...
  if(calc.turnover==T) return(turnover[2:nrow(turnover),]) else{
    if(netlong==2) return(list(cmom[2:nrow(cmom),],netlongpos[2:nrow(cmom),],long[2:nrow(cmom),])) else{
...
bah <- function(data, scaled=F, calc.turnover=F){
  #Input: x - data.frame of daily prices
  #Output: data.frame with time series of bah returns in columns
  return(cmom(data, scaled, th=-1000000, calc.turnover))
}

    ...
    for(h in holdper){
      for(mon.index in 2:(nrow(x)-r-h+1)){
        ...
        if(netlong==1 | netlong==4){
          ...
          tvm[mon.index,c] <- sum(prod.ret.hp[is.na(prod.ret.hp)==F & is.na(weights)==F]*weights[is.na(prod.ret.hp)==F & is.na(weights)==F])/sum(weights[is.na(prod.ret.hp)==F & is.na(weights)==F])
      ...
      if(length(is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp>=th & is.na(weights)==F)==0) companies.long.new <- 0
      if(length(is.na(prod.ret.hp)==F & is.na(prod.ret.rp)==F & prod.ret.rp<th & is.na(weights)==F)==0) companies.short.new <- 0
...
#---------------------------------------------------------TS
tmom <- function(data=ps, scaled=F, netlong=2){
  ...
  return(cmom(data, scaled, netlong, th=0))
}
